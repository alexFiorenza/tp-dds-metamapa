...
/**
 * Implementación del algoritmo de mayoría simple.
 * Si al menos la mitad de las fuentes contienen el mismo hecho,
 * se lo considera consensuado.
 */
public class ConsensoSimple implements AlgoritmoConsenso {
    @Override
    public List<Hecho> filtrarHechosConsensuados(List<Hecho> hechos, List<Fuente> fuentes) {
        ...
        int mitadFuentes = (totalFuentes + 1) / 2; // Redondeo hacia arriba
        ...
        for (Map.Entry<String, List<Hecho>> entry : hechosPorTitulo.entrySet()) {
            ...
            // Verificar que al menos la mitad de las fuentes tienen este hecho
            if (fuentesConEsteTitulo >= mitadFuentes) {
